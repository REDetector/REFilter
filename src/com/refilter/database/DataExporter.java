...
public class DataExporter {
    ...
    public void exportData(String resultPath, String databaseName, String mode, String[] columns, String selection, String[] selectionArgs) {
        ...
        for (String currentTable : currentTables) {
            ...
            if (columns.length == 1 && columns[0].equalsIgnoreCase("all")) {
                if (currentTable.contains(DatabaseManager.RNA_VCF_RESULT_TABLE_NAME) && !currentTable.contains(DatabaseManager.FILTER)) {
                ...
                if (currentTable.contains(DatabaseManager.FET_FILTER_RESULT_TABLE_NAME)) {
                    ...
                    try {
                        while (rs.next()) {
                            pw.println(rs.getString(1).substring(3) + "\t" + rs.getInt(2) + "\t" + rs.getInt(2) + "\t" + rs.getString(3) + "\t" + rs
                                    .getString(4));
                        }
                    } catch (SQLException e) {
                        logger.warn("No results", e);
    ...
    public void exportStrandResults(File inputFile, File outputFile) throws IOException, SQLException {
        ...
        for (StrandSite strandSite : strandSites) {
            ResultSet rs = databaseManager.query(DatabaseManager.SPLICE_JUNCTION_TABLE_NAME, new String[]{"strand"}, "chrom=? and begin<=? and end>=?", new
                    String[]{strandSite.chr, strandSite.pos, strandSite.pos});
            if (rs.next()) {
    ...
    public void exportDNAResults(String dnaVCF, String[] chrPoses, File outputFile) throws FileNotFoundException, SQLException {
        ...
        for (int i = 0, len = chrPoses.length; i < len; i += 2) {
            ResultSet rs = databaseManager.query(dnaVCF, new String[]{"ref"}, "chrom=? and pos=?", new String[]{chrPoses[i], chrPoses[i + 1]});
            if (rs.next()) {
